    ...
    internal class MP3toOGG
    {
        ...
        public static string ConvertMp3toOgg(string mp3Path, string outputDir)
        {
            ...
            try
            {
                ...
                var songTitle = tagFile.Tag.Title;
                var artist = tagFile.Tag.AlbumArtists.FirstOrDefault();
                var albumTitle = tagFile.Tag.Album;

                //System.Windows.MessageBox.Show("songTitle: " + songTitle + ". artist: " + artist + ". albumTitle: " + albumTitle);
                ...
                using (var ogg = new FileStream(Path.Combine(outputDir, oggName), FileMode.Create, FileAccess.Write))
                {
                    ...
                    if (songTitle != null) comments.AddTag("TITLE", songTitle);
                    if (artist != null) comments.AddTag("ARTIST", artist);
                    if (albumTitle != null) comments.AddTag("ALBUM", albumTitle);
